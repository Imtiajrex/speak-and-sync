...
export const useVoiceCommands = () => {
  ...
  const processCommand = async (transcript: string): Promise<void> => {
    ...
    try {
      // Simulate API call to LLM for command parsing
      // In a real app, this would call OpenAI, Claude, or similar
      const response = await simulateAIProcessing(transcript);
...
const simulateAIProcessing = async (text: string): Promise<ParsedCommand> => {
  ...
  const patterns = [
    ...
    {
      pattern: /set (?:the )?(.+?) (?:to )?(\d+(?:\.\d+)?)\s*(?:degrees?|°f?|°c?)?/,
      extract: (match: RegExpMatchArray): Omit<ParsedCommand, 'room'> => ({
